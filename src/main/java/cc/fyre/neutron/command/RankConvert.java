package cc.fyre.neutron.command;
...
public class RankConvert {
    ...
    public static void ranks(CommandSender sender) {
        ...
        final Neutron instance = Neutron.getInstance();
        ...
        database.getCollection("Documentation").find().iterator().forEachRemaining(it -> {
            ...
            profile.getGrants().add(new Grant(Neutron.getInstance().getRankHandler().getDefaultRank(), UUIDCache.CONSOLE_UUID,(long)Integer.MAX_VALUE,"Default Grant"));
